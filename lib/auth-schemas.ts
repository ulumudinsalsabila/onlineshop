...
const internationalPhone = z.string().trim().regex(/^\+[1-9]\d{7,14}$/, "Enter a valid phone number.");
export const registerSchema = z.object({ name: z.string().trim().min(2).max(80), email: z.string().trim().email().transform(normalizeEmail), phone: internationalPhone, password, confirmPassword: z.string() }).refine((value) => value.password === value.confirmPassword, { message: "The password confirmation does not match.", path: ["confirmPassword"] });
