...
export const productAdminSchema = z.object({
  ...
  variants: z.array(z.object({ id: z.string().cuid().optional(), sku: z.string().trim().min(2).max(80).regex(/^[A-Z0-9_-]+$/i), name: clean(100), color: optionalText(60), colorHex: z.string().regex(/^#[0-9a-f]{6}$/i).optional().or(z.literal("")), size: optionalText(30), price: money.optional().nullable(), compareAtPrice: money.optional().nullable(), weightInGrams: z.coerce.number().int().positive().max(1_000_000), stock: z.coerce.number().int().nonnegative().max(1_000_000), lowStockAt: z.coerce.number().int().nonnegative().max(10_000).default(2), isActive: z.boolean().default(true) })).min(1).max(100),
});
