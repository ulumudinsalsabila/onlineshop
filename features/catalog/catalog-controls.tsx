...
export function CatalogControls({ query, total, suggestions, config, children }: CatalogControlsProps) {
  ...
  const [searchDraft, setSearchDraft] = useState({ source: query.q, value: query.q });
  ...
  function navigate(next: CatalogQuery, mode: "push" | "replace" = "push") {
    const params = queryToSearchParams({ ...next, page: Math.max(1, next.page) });
      ...
      <div className={cn("relative z-20", config.searchMode ? "mt-8 max-w-3xl" : "mt-8 max-w-2xl")}>
        ...
        >
          ...
          <div className="relative border-b border-foreground/35">
            ...
            />
            <button type="submit" className="absolute top-1/2 right-0 -translate-y-1/2 text-[0.625rem] font-semibold tracking-[0.14em] uppercase focus-visible:rounded-sm focus-visible:outline-2 focus-visible:outline-ring">Search</button>
          </div>
        </form>
            ...
            <m.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }} className="absolute inset-x-0 top-full border border-border bg-popover p-3 shadow-lifted">
              ...
              <ul>
                {visibleSuggestions.map((item) => (
                  <li key={item}>
                    <button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => { setSearchDraft({ source: query.q, value: item }); rememberSearch(item); setSearchFocused(false); navigate({ ...query, q: item, page: 1 }); }} className="flex w-full items-center gap-3 px-3 py-2.5 text-left text-sm transition-colors hover:bg-secondary focus-visible:outline-2 focus-visible:outline-ring">
              ...
              <div className="flex flex-wrap gap-2 pt-4">
                {chips.map((chip) => (
                  <m.button layout key={chip.key} type="button" onClick={chip.remove} initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.96 }} className="inline-flex h-8 items-center gap-2 rounded-full border border-border bg-secondary/60 px-3 text-xs focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring">
                    {chip.label}<XIcon size={13} aria-hidden />
                  </m.button>
                ))}
                <button type="button" onClick={() => patch(resetFilters(query))} className="px-2 text-xs font-semibold underline underline-offset-4">Reset filters</button>
              </div>
            </m.div>
          ) : null}
        </AnimatePresence>
      ...
      <Sheet open={mobileOpen} onOpenChange={setMobileOpen}>
        <SheetContent side="left" className="w-[min(92vw,27rem)] gap-0 sm:max-w-md">
          ...
          <SheetFooter className="grid grid-cols-2 border-t border-border p-4">
            <Button variant="outline" onClick={() => setDraft((current) => ({ ...current, ...resetFilters(current) }))}>Reset</Button>
            <Button onClick={() => { setMobileOpen(false); navigate({ ...query, ...draft, page: 1 }); }}><CheckIcon aria-hidden /> Apply filters</Button>
          </SheetFooter>
        </SheetContent>
      </Sheet>
    </>
...
function FilterFields({ value, locked, onChange }: { value: CatalogQuery; locked: Set<string>; onChange: (patch: Partial<CatalogQuery>) => void }) {
  return (
    <Accordion type="multiple" defaultValue={["category", "brand", "condition", "price", "color", "size", "availability", "discount"]}>
      {!locked.has("category") ? <FilterSection value="category" title="Category"><CheckList options={catalogCategories} selected={value.categories} onChange={(categories) => onChange({ categories })} /></FilterSection> : null}
      {!locked.has("brand") ? <FilterSection value="brand" title="Brand"><CheckList options={catalogBrands} selected={value.brands} onChange={(brands) => onChange({ brands })} /></FilterSection> : null}
      ...
      <FilterSection value="color" title="Color"><CheckList options={catalogColors} selected={value.colors} onChange={(colors) => onChange({ colors })} swatches /></FilterSection>
      <FilterSection value="size" title="Size">
        <div className="grid grid-cols-4 gap-2">{catalogSizes.map((size) => { const active = value.sizes.includes(size); return <button key={size} type="button" aria-pressed={active} onClick={() => onChange({ sizes: toggle(value.sizes, size) })} className={cn("min-h-9 border px-1 text-xs transition-colors focus-visible:outline-2 focus-visible:outline-ring", active ? "border-primary bg-primary text-primary-foreground" : "border-border hover:bg-secondary", size === "One Size" && "col-span-2")}>{size}</button>; })}</div>
      </FilterSection>
      <FilterSection value="availability" title="Availability"><label className="flex cursor-pointer items-center gap-3 py-2 text-sm"><Checkbox checked={value.inStock} onCheckedChange={(checked) => onChange({ inStock: checked === true })} /> In stock only</label></FilterSection>
      {!locked.has("discount") ? <FilterSection value="discount" title="Discount"><div className="grid grid-cols-2 gap-2">{[0, 10, 20, 30].map((discount) => <button key={discount} type="button" aria-pressed={value.minDiscount === discount} onClick={() => onChange({ minDiscount: discount })} className={cn("border border-border px-3 py-2 text-xs transition-colors focus-visible:outline-2 focus-visible:outline-ring", value.minDiscount === discount && "border-primary bg-primary text-primary-foreground")}>{discount ? `${discount}%+` : "All"}</button>)}</div></FilterSection> : null}
    </Accordion>
  );
}

function FilterSection({ value, title, children }: { value: string; title: string; children: React.ReactNode }) {
  return <AccordionItem value={value} className="border-border"><AccordionTrigger className="rounded-none py-4 text-xs font-semibold tracking-[0.12em] uppercase hover:no-underline">{title}</AccordionTrigger><AccordionContent className="pb-5">{children}</AccordionContent></AccordionItem>;
}
...
function CheckList({ options, selected, onChange, swatches = false }: { options: ReadonlyArray<{ value: string; label: string; swatch?: string }>; selected: string[]; onChange: (items: string[]) => void; swatches?: boolean }) {
  return <div className="space-y-1">{options.map((option) => <label key={option.value} className="flex cursor-pointer items-center gap-3 py-2 text-sm"><Checkbox checked={selected.includes(option.value)} onCheckedChange={() => onChange(toggle(selected, option.value))} />{swatches ? <span className="size-3 rounded-full border border-black/10" style={{ backgroundColor: option.swatch }} aria-hidden /> : null}<span>{option.label}</span></label>)}</div>;
}
...
function resetFilters(query: CatalogQuery): Partial<CatalogQuery> {
  return { categories: [], brands: [], conditions: [], minPrice: CATALOG_MIN_PRICE, maxPrice: CATALOG_MAX_PRICE, colors: [], sizes: [], inStock: false, minDiscount: 0, page: 1, q: query.q, sort: query.sort };
}
...
function createChips(query: CatalogQuery, locked: Set<string>, patch: (value: Partial<CatalogQuery>) => void) {
  ...
  if (!locked.has("category")) query.categories.forEach((value) => chips.push({ key: `category-${value}`, label: catalogCategories.find((item) => item.value === value)?.label ?? value, remove: () => patch({ categories: query.categories.filter((item) => item !== value) }) }));
  if (!locked.has("brand")) query.brands.forEach((value) => chips.push({ key: `brand-${value}`, label: catalogBrands.find((item) => item.value === value)?.label ?? value, remove: () => patch({ brands: query.brands.filter((item) => item !== value) }) }));
  if (!locked.has("condition")) query.conditions.forEach((value) => chips.push({ key: `condition-${value}`, label: value === "new" ? "New" : "Preloved", remove: () => patch({ conditions: query.conditions.filter((item) => item !== value) }) }));
  query.colors.forEach((value) => chips.push({ key: `color-${value}`, label: catalogColors.find((item) => item.value === value)?.label ?? value, remove: () => patch({ colors: query.colors.filter((item) => item !== value) }) }));
  query.sizes.forEach((value) => chips.push({ key: `size-${value}`, label: `Size ${value}`, remove: () => patch({ sizes: query.sizes.filter((item) => item !== value) }) }));
  if (query.minPrice > CATALOG_MIN_PRICE || query.maxPrice < CATALOG_MAX_PRICE) chips.push({ key: "price", label: `${formatIDR(query.minPrice)} – ${formatIDR(query.maxPrice)}`, remove: () => patch({ minPrice: CATALOG_MIN_PRICE, maxPrice: CATALOG_MAX_PRICE }) });
  if (query.inStock) chips.push({ key: "stock", label: "In stock", remove: () => patch({ inStock: false }) });
  if (!locked.has("discount") && query.minDiscount) chips.push({ key: "discount", label: `Discount ${query.minDiscount}%+`, remove: () => patch({ minDiscount: 0 }) });
  return chips;
}
