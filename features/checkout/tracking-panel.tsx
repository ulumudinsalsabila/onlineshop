...
export function TrackingPanel({ orderId, initial }: { orderId: string; initial: Tracking }) {
  ...
  async function refresh() { setBusy(true); try { const response = await apiFetch(`/api/orders/${orderId}/tracking`, { cache: "no-store" }); const result = await response.json() as { success?: boolean; data?: Tracking; error?: { message?: string } }; if (!result.success || !result.data) throw new Error(result.error?.message); setTracking(result.data); } catch (error) { toast.error(error instanceof Error ? error.message : "Tracking could not be loaded."); } finally { setBusy(false); } }
