...
type Address = { id: string; label: string; recipient: string; phone: string; line1: string; line2?: string | null; village?: string | null; district: string; city: string; province: string; postalCode: string; country: string; isDefault: boolean };
type Item = { variantId: string; productName: string; variantName: string; quantity: number; unitPrice: number; stock: number };
type Context = { cartId: string; customer: { name: string | null; email: string; phone: string | null }; addresses: Address[]; items: Item[] };
type Destination = { id: number; label: string; province: string; city: string; district: string; subdistrict: string; postalCode: string };
type Quote = { provider: string; courierCode: string; courierName: string; serviceCode: string; serviceName: string; description: string; cost: number; etd: string; estimateLabel: string };
...
const emptyAddress = { label: "Home", recipient: "", phone: "", line1: "", line2: "", postalCode: "", country: "Indonesia", isDefault: false, provinceCode: "", regencyCode: "", districtCode: "", villageCode: "" };
...
export function CheckoutFlow() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      void apiFetch(`/api/shipping/destinations?search=${encodeURIComponent(query)}`, { cache: "no-store", signal: controller.signal }).then(async (response) => {
        ...
        setDestinations(result.data.items); setDestinationState(result.data.items.length ? "ready" : "empty");
      }).catch((error: unknown) => {
  ...
  function changeDestinationQuery(value: string) {
    setDestinationQuery(value); setDestination(null); setDestinations([]); setQuotes([]); setQuote(null); setRateError(null); setTotals(null);
    setDestinationState(value.trim().length < 3 ? "idle" : "loading");
  }

  async function loadRates() {
    if (!destination) { toast.error("Select a destination district or city first."); return; }
    if (!context?.cartId) { setRateError({ code: "CART_CONTEXT_STALE", message: "The cart is not synchronized. Reload this page after restarting the backend." }); return; }
    setBusy(true);
    setRateError(null); setQuotes([]); setQuote(null);
    try {
      const response = await apiFetch("/api/shipping/rates", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ destinationId: destination.id, cartId: context.cartId }) });
      const result = await response.json() as ApiResult<{ rates: Quote[] }>;
      if (!result.success) { setRateError(result.error); return; }
      setQuotes(result.data.rates);
    } catch { setRateError({ code: "NETWORK_ERROR", message: "The connection to the shipping service was interrupted." }); } finally { setBusy(false); }
  }
  ...
  const payload = useMemo(() => quote && destination ? { addressId, shipping: { destinationId: destination.id, courierCode: quote.courierCode, serviceCode: quote.serviceCode }, voucherCode: voucherCode || undefined, paymentMethod, notes: notes || undefined } : null, [addressId, destination, notes, paymentMethod, quote, voucherCode]);
  async function preview() {
    ...
    try {
      ...
      if (!result.success) throw new Error(result.error.message);
      setTotals(result.data); toast.success(voucherCode ? "Voucher applied." : "Total updated.");
  ...
  async function next() {
    ...
    if (step === 2 && !quote) { toast.error("Check the shipping rates and select a shipping service."); return; }
  ...
  return <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_22rem]">
    <section>
      <ol aria-label="Checkout steps" className="mb-10 grid grid-cols-7 gap-1">{steps.map((label, index) => <li key={label} className="min-w-0"><span className={`block h-1 ${index <= step ? "bg-primary" : "bg-border"}`} /><span className="mt-2 hidden text-[0.625rem] tracking-wider uppercase md:block">{label}</span></li>)}</ol>
      <m.div key={step} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.22 }}>
        ...
        <div className="mt-8">{step === 0 && <CustomerStep context={context} notes={notes} setNotes={setNotes} />}{step === 1 && <AddressStep addresses={context.addresses} addressId={addressId} setAddressId={(value) => { setAddressId(value); setQuote(null); setQuotes([]); setTotals(null); }} adding={addingAddress} setAdding={setAddingAddress} form={addressForm} setForm={setAddressForm} save={saveAddress} busy={busy} />}{step === 2 && <ShippingStep query={destinationQuery} setQuery={changeDestinationQuery} destinations={destinations} destination={destination} chooseDestination={(value) => { setDestination(value); setDestinationQuery(value.label); setDestinations([]); setDestinationState("ready"); setTotals(null); }} destinationState={destinationState} quotes={quotes} quote={quote} setQuote={(value) => { setQuote(value); setTotals(null); }} checkRates={loadRates} rateError={rateError} busy={busy} />}{step === 3 && <VoucherStep code={voucherCode} setCode={setVoucherCode} apply={preview} busy={busy} />}{step === 4 && <PaymentStep value={paymentMethod} setValue={setPaymentMethod} />}{step === 5 && <ReviewStep context={context} address={selectedAddress} quote={quote} payment={paymentMethod} />}{step === 6 && <ConfirmationStep submit={submitOrder} busy={busy} />}</div>
      </m.div>
...
function AddressStep({ addresses, addressId, setAddressId, adding, setAdding, form, setForm, save, busy }: { addresses: Address[]; addressId: string; setAddressId(value: string): void; adding: boolean; setAdding(value: boolean): void; form: typeof emptyAddress; setForm(value: typeof emptyAddress): void; save(): void; busy: boolean }) { const setRegions = (selection: RegionSelection) => setForm({ ...form, ...selection }); return <div><div className="grid gap-3 sm:grid-cols-2">{addresses.map((address) => <button type="button" key={address.id} aria-pressed={addressId === address.id} onClick={() => setAddressId(address.id)} className={`border p-5 text-left transition-colors ${addressId === address.id ? "border-primary bg-secondary/60" : "border-border hover:border-foreground/40"}`}><MapPinIcon className="mb-3 size-5" aria-hidden /><p className="font-semibold">{address.label} · {address.recipient}</p><p className="mt-2 text-sm leading-6 text-muted-foreground">{address.line1}, {address.village ? `${address.village}, ` : ""}{address.district}, {address.city} {address.postalCode}<br />{address.phone}</p></button>)}</div><Button className="mt-5" variant="outline" onClick={() => setAdding(!adding)} aria-expanded={adding}>Add address</Button>{adding && <div className="mt-6 grid gap-4 border border-border p-5 sm:grid-cols-2">{(["label", "recipient", "phone", "line1", "postalCode"] as const).map((key) => <div key={key} className={key === "line1" ? "sm:col-span-2" : ""}><Label htmlFor={`address-${key}`}>{({ label: "Label", recipient: "Recipient", phone: "Phone number", line1: "Address", postalCode: "Postal code" })[key]}</Label><Input id={`address-${key}`} value={form[key]} inputMode={key === "postalCode" ? "numeric" : undefined} onChange={(event) => setForm({ ...form, [key]: event.target.value })} className="mt-2" /></div>)}<RegionFields onSelectionChange={setRegions} /><Button disabled={busy} onClick={save} className="sm:col-span-2">Save address</Button></div>}</div>; }
function ShippingStep({ query, setQuery, destinations, destination, chooseDestination, destinationState, quotes, quote, setQuote, checkRates, rateError, busy }: { query: string; setQuery(value: string): void; destinations: Destination[]; destination: Destination | null; chooseDestination(value: Destination): void; destinationState: DestinationState; quotes: Quote[]; quote: Quote | null; setQuote(value: Quote): void; checkRates(): void; rateError: { code: string; message: string } | null; busy: boolean }) {
  ...
  return <div className="space-y-6"><div className="relative"><Label htmlFor="shipping-destination">District, city, or postal code</Label><Input id="shipping-destination" value={query} onChange={(event) => setQuery(event.target.value)} autoComplete="off" placeholder="Example: Coblong, Bandung, 40132" className="mt-2" aria-expanded={destinations.length > 0} aria-controls="destination-results" />{stateMessage && <p role="status" className="mt-2 text-xs text-muted-foreground">{stateMessage}</p>}{destinations.length > 0 && <div id="destination-results" className="absolute z-20 mt-1 max-h-64 w-full overflow-auto border border-border bg-background shadow-lg">{destinations.map((item) => <button type="button" key={item.id} onClick={() => chooseDestination(item)} className="block w-full border-b border-border px-4 py-3 text-left text-sm last:border-0 hover:bg-secondary"><span className="font-medium">{item.label}</span><span className="mt-1 block text-xs text-muted-foreground">{item.district}, {item.city} {item.postalCode}</span></button>)}</div>}</div><Button type="button" onClick={checkRates} disabled={!destination || busy}>{busy && <SpinnerGapIcon className="animate-spin" aria-hidden />}Check shipping rates</Button>{destination && <p className="text-xs text-muted-foreground">Selected destination: {destination.label}</p>}{rateMessage && <div role="alert" className="border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">{rateMessage}</div>}{!busy && destination && !rateError && quotes.length === 0 && <p className="text-sm text-muted-foreground">Click “Check shipping rates” to display available services.</p>}<div className="grid gap-3">{quotes.map((item) => { const selected = quote?.courierCode === item.courierCode && quote.serviceCode === item.serviceCode; return <label key={`${item.courierCode}-${item.serviceCode}`} className={`flex cursor-pointer items-center justify-between gap-4 border p-5 ${selected ? "border-primary bg-secondary/60" : "border-border"}`}><span className="flex gap-3"><input type="radio" name="shipping-rate" checked={selected} onChange={() => setQuote(item)} /><span><span className="font-semibold">{item.courierName} · {item.serviceName}</span><span className="mt-1 block text-xs text-muted-foreground">{item.description || "Shipping service"} · Estimated {item.etd || item.estimateLabel}</span></span></span><span className="font-medium">{formatIDR(item.cost)}</span></label>; })}</div></div>;
}
