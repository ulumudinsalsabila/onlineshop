...
const steps = ["Informasi", "Alamat", "Pengiriman", "Voucher", "Pembayaran", "Review", "Konfirmasi"];
...
export function CheckoutFlow() {
  ...
  async function saveAddress() {
    ...
    try {
      ...
      if (!result.success) throw new Error(result.error.message);
      await loadContext(); setAddressId(result.data.id); setAddingAddress(false); toast.success("Alamat disimpan.");
    } catch (error) { toast.error(error instanceof Error ? error.message : "Alamat tidak valid."); } finally { setBusy(false); }
  }
  ...
  async function preview() {
    ...
    } catch (error) { setTotals(null); toast.error(error instanceof Error ? error.message : "Tidak dapat menghitung total."); } finally { setBusy(false); }
  }
  async function submitOrder() {
    ...
    } catch (error) { toast.error(error instanceof Error ? error.message : "Pesanan gagal dibuat."); setBusy(false); }
  }

  async function next() {
    if (step === 1 && !addressId) { toast.error("Pilih alamat pengiriman."); return; }
    if (step === 1) await loadRates();
    if (step === 2 && !quote) { toast.error("Pilih layanan pengiriman."); return; }
    if (step === 4 || step === 5) await preview();
    setStep((current) => Math.min(6, current + 1));
  }

  if (!context) return <div className="flex min-h-[28rem] items-center justify-center"><SpinnerGapIcon className="size-7 animate-spin" aria-hidden /><span className="sr-only">Memuat checkout</span></div>;
  ...
  return <div className="grid gap-10 lg:grid-cols-[minmax(0,1fr)_22rem]">
    <section>
      ...
      <m.div key={step} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.22 }}>
        <p className="text-xs tracking-[0.18em] text-muted-foreground uppercase">Langkah {step + 1} dari 7</p>
      ...
      <div className="mt-10 flex justify-between border-t border-border pt-6"><Button variant="ghost" disabled={step === 0 || busy} onClick={() => setStep((current) => current - 1)}><ArrowLeftIcon aria-hidden /> Kembali</Button>{step < 6 && <Button disabled={busy} onClick={() => void next()}>{busy && <SpinnerGapIcon className="animate-spin" aria-hidden />} Lanjutkan <ArrowRightIcon aria-hidden /></Button>}</div>
    </section>
    <aside className="h-fit border border-border bg-secondary/30 p-6 lg:sticky lg:top-28"><h2 className="font-serif text-2xl">Ringkasan pesanan</h2><div className="mt-5 space-y-4">{context.items.map((item) => <div key={item.variantId} className="flex justify-between gap-4 text-sm"><div><p>{item.productName}</p><p className="text-xs text-muted-foreground">{item.variantName} · {item.quantity}</p></div><span className="tabular-nums">{formatIDR(item.unitPrice * item.quantity)}</span></div>)}</div><dl className="mt-6 space-y-3 border-t border-border pt-5 text-sm"><SummaryRow label="Subtotal" value={rawSubtotal} /><SummaryRow label="Diskon" value={totals?.discountTotal ?? 0} /><SummaryRow label="Pengiriman" value={totals?.shippingTotal ?? quote?.cost ?? 0} /><SummaryRow label="Total" value={totals?.grandTotal ?? rawSubtotal + (quote?.cost ?? 0)} strong /></dl><p className="mt-4 text-[0.6875rem] leading-5 text-muted-foreground">Total final dihitung ulang oleh server saat pesanan dibuat.</p></aside>
  </div>;
}

function CustomerStep({ context, notes, setNotes }: { context: Context; notes: string; setNotes(value: string): void }) { return <div className="space-y-6"><div className="border border-border p-5"><p className="font-medium">{context.customer.name ?? "Customer"}</p><p className="mt-1 text-sm text-muted-foreground">{context.customer.email}</p></div><div><Label htmlFor="notes">Catatan pesanan (opsional)</Label><textarea id="notes" value={notes} maxLength={500} onChange={(event) => setNotes(event.target.value)} className="mt-2 min-h-28 w-full rounded-md border border-input bg-card p-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring/30" placeholder="Instruksi khusus untuk pesanan Anda" /></div></div>; }
function AddressStep({ addresses, addressId, setAddressId, adding, setAdding, form, setForm, save, busy }: { addresses: Address[]; addressId: string; setAddressId(value: string): void; adding: boolean; setAdding(value: boolean): void; form: typeof emptyAddress; setForm(value: typeof emptyAddress): void; save(): void; busy: boolean }) { return <div><div className="grid gap-3 sm:grid-cols-2">{addresses.map((address) => <button type="button" key={address.id} aria-pressed={addressId === address.id} onClick={() => setAddressId(address.id)} className={`border p-5 text-left transition-colors ${addressId === address.id ? "border-primary bg-secondary/60" : "border-border hover:border-foreground/40"}`}><MapPinIcon className="mb-3 size-5" aria-hidden /><p className="font-semibold">{address.label} · {address.recipient}</p><p className="mt-2 text-sm leading-6 text-muted-foreground">{address.line1}, {address.district}, {address.city} {address.postalCode}<br />{address.phone}</p></button>)}</div><Button className="mt-5" variant="outline" onClick={() => setAdding(!adding)} aria-expanded={adding}>Tambah alamat</Button>{adding && <div className="mt-6 grid gap-4 border border-border p-5 sm:grid-cols-2">{(["label", "recipient", "phone", "line1", "district", "city", "province", "postalCode"] as const).map((key) => <div key={key} className={key === "line1" ? "sm:col-span-2" : ""}><Label htmlFor={`address-${key}`}>{({ label: "Label", recipient: "Penerima", phone: "Nomor telepon", line1: "Alamat", district: "Kecamatan", city: "Kota/Kabupaten", province: "Provinsi", postalCode: "Kode pos" })[key]}</Label><Input id={`address-${key}`} value={form[key]} onChange={(event) => setForm({ ...form, [key]: event.target.value })} className="mt-2" /></div>)}<Button disabled={busy} onClick={save} className="sm:col-span-2">Simpan alamat</Button></div>}</div>; }
function ShippingStep({ quotes, quote, setQuote, busy }: { quotes: Quote[]; quote: Quote | null; setQuote(value: Quote): void; busy: boolean }) { if (busy) return <p role="status" aria-live="polite">Memuat pilihan kurir…</p>; return <div className="grid gap-3">{quotes.map((item) => { const selected = quote?.courierCode === item.courierCode && quote.serviceCode === item.serviceCode; return <button type="button" key={`${item.courierCode}-${item.serviceCode}`} aria-pressed={selected} onClick={() => setQuote(item)} className={`flex items-center justify-between border p-5 text-left ${selected ? "border-primary bg-secondary/60" : "border-border"}`}><span><span className="font-semibold">{item.courierName} · {item.serviceName}</span><span className="mt-1 block text-xs text-muted-foreground">Estimasi {item.estimateLabel}</span></span><span className="font-medium">{formatIDR(item.cost)}</span></button>; })}</div>; }
function VoucherStep({ code, setCode, apply, busy }: { code: string; setCode(value: string): void; apply(): void; busy: boolean }) { return <div className="max-w-lg"><Label htmlFor="voucher">Kode voucher</Label><div className="mt-2 flex gap-2"><Input id="voucher" value={code} onChange={(event) => setCode(event.target.value.toUpperCase())} placeholder="WELCOME10" /><Button variant="outline" disabled={!code || busy} onClick={apply}>Terapkan</Button></div><p className="mt-3 text-xs text-muted-foreground">Validitas, periode, minimum belanja, dan kuota diverifikasi kembali di server.</p></div>; }
function PaymentStep({ value, setValue }: { value: string; setValue(value: string): void }) { const methods = [["VIRTUAL_ACCOUNT", "Virtual account"], ["BANK_TRANSFER", "Bank transfer"], ["E_WALLET", "E-wallet"], ["CREDIT_CARD", "Kartu kredit/debit"]]; return <div className="grid gap-3 sm:grid-cols-2">{methods.map(([id, label]) => <button type="button" key={id} aria-pressed={value === id} onClick={() => setValue(id)} className={`border p-5 text-left ${value === id ? "border-primary bg-secondary/60" : "border-border"}`}><span className="font-semibold">{label}</span><span className="mt-2 block text-xs text-muted-foreground">Diproses aman melalui penyedia pembayaran.</span></button>)}</div>; }
function ReviewStep({ context, address, quote, payment }: { context: Context; address?: Address; quote: Quote | null; payment: string }) { return <div className="grid gap-4 sm:grid-cols-2"><ReviewBox title="Customer" text={`${context.customer.name ?? "Customer"}\n${context.customer.email}`} /><ReviewBox title="Alamat" text={address ? `${address.recipient}\n${address.line1}, ${address.city} ${address.postalCode}` : "-"} /><ReviewBox title="Pengiriman" text={quote ? `${quote.courierName} ${quote.serviceName}\n${quote.estimateLabel}` : "-"} /><ReviewBox title="Pembayaran" text={payment.replaceAll("_", " ")} /></div>; }
function ConfirmationStep({ submit, busy }: { submit(): void; busy: boolean }) { return <div className="border border-border bg-secondary/30 p-7 text-center"><CheckCircleIcon className="mx-auto size-10" weight="thin" aria-hidden /><h2 className="mt-4 font-serif text-3xl">Siap membuat pesanan?</h2><p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-muted-foreground">Stok akan direservasi, semua nilai dihitung ulang, lalu Anda diarahkan ke pembayaran.</p><Button size="lg" className="mt-7" disabled={busy} onClick={submit}>{busy ? <SpinnerGapIcon className="animate-spin" aria-hidden /> : <PackageIcon aria-hidden />} Buat pesanan & bayar</Button></div>; }
