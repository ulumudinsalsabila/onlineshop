...
export function PaymentActions({ orderId, mock }: { orderId: string; mock: boolean }) {
  ...
  return <div className="mt-7 flex flex-wrap justify-center gap-3"><Button disabled={busy} onClick={() => void call(`/api/payments/${orderId}/sync`)} variant="outline">{busy ? <SpinnerGapIcon className="animate-spin" aria-hidden /> : <ArrowClockwiseIcon aria-hidden />} Sync status</Button>{mock && <><Button disabled={busy} onClick={() => void call(`/api/payments/mock/${orderId}`, { status: "settlement" })}><CheckIcon aria-hidden /> Simulate success</Button><Button disabled={busy} variant="destructive" onClick={() => void call(`/api/payments/mock/${orderId}`, { status: "failure" })}><XIcon aria-hidden /> Simulate failure</Button></>}</div>;
}
