...
export function AuthForm({ mode, callbackUrl = "/account", email = "", token = "", verified = false, invalidToken = false }: { mode: Mode; callbackUrl?: string; email?: string; token?: string; verified?: boolean; invalidToken?: boolean }) {
  ...
  const [message, setMessage] = useState(verified ? "Email berhasil diverifikasi. Silakan masuk." : invalidToken ? "Link verifikasi tidak valid atau sudah kedaluwarsa." : "");
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      if (mode === "login") {
        ...
        if (result?.error) throw new Error("Email/password tidak sesuai, akun belum diverifikasi, atau percobaan terlalu banyak.");
      ...
      if (!response.ok || !result.success) {
        ...
        throw new Error(firstFieldError ?? result.error?.message ?? "Permintaan tidak dapat diproses.");
      }
      setSuccess(true); setMessage(result.data?.message ?? "Berhasil.");
      if (mode === "reset") window.setTimeout(() => router.push("/login"), 1200);
    } catch (error) { setMessage(error instanceof Error ? error.message : "Terjadi kesalahan."); } finally { setPending(false); }
  }

  return (
    <form onSubmit={submit} className="mt-8 space-y-5">
      {mode === "register" ? <Field id="name" label="Nama lengkap" autoComplete="name" /> : null}
      {mode !== "reset" ? <Field id="email" label="Email" type="email" autoComplete="email" /> : <p className="border border-border bg-secondary/40 px-4 py-3 text-sm">Reset password untuk <strong>{email}</strong></p>}
      {mode === "login" || mode === "register" || mode === "reset" ? (
        <div><Label htmlFor="password">Password</Label><div className="relative mt-2"><Input id="password" name="password" type={showPassword ? "text" : "password"} autoComplete={mode === "login" ? "current-password" : "new-password"} required minLength={mode === "login" ? 1 : 10} className="pr-12" /><button type="button" onClick={() => setShowPassword((value) => !value)} aria-label={showPassword ? "Sembunyikan password" : "Tampilkan password"} className="absolute inset-y-0 right-0 grid w-11 place-items-center"><span aria-hidden>{showPassword ? <EyeSlashIcon /> : <EyeIcon />}</span></button></div></div>
      ) : null}
      {mode === "register" || mode === "reset" ? <Field id="confirmPassword" label="Konfirmasi password" type={showPassword ? "text" : "password"} autoComplete="new-password" minLength={10} /> : null}
      ...
      <Button type="submit" size="lg" className="w-full" disabled={pending}>{pending ? "Memproses…" : mode === "login" ? "Sign in" : mode === "register" ? "Create account" : mode === "forgot" ? "Send reset link" : "Update password"}<ArrowRightIcon aria-hidden /></Button>
      ...
      {mode !== "login" ? <p className="text-center text-xs text-muted-foreground">Sudah memiliki akun? <Link href="/login" className="text-foreground underline underline-offset-4">Sign in</Link></p> : null}
    </form>
  );
}
