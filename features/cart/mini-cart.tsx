    ...
    <Sheet open={cartOpen} onOpenChange={setCartOpen}>
      <SheetContent side="right" className="w-[min(94vw,29rem)] gap-0 sm:max-w-md">
        <SheetHeader className="border-b border-border p-5 pr-14">
          ...
          <SheetDescription>{hydrated ? `${cartCount} items in your bag` : "Loading shopping bag…"}</SheetDescription>
        </SheetHeader>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          {hydrated && !cartLines.length ? (
            <div className="flex min-h-80 flex-col items-center justify-center text-center">
              ...
              <p className="mt-2 max-w-xs text-sm leading-6 text-muted-foreground">Find the piece that feels right and keep it here until checkout.</p>
              ...
              <m.article key={line.id} layout initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 18, height: 0 }} className="grid grid-cols-[6.5rem_1fr] gap-4 border-b border-border py-5 first:pt-0">
                ...
                <div className="min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button type="button" onClick={() => removeFromCart(line.id)} aria-label={`Remove ${line.product.name}`} className="grid size-8 shrink-0 place-items-center text-muted-foreground hover:text-destructive focus-visible:outline-2 focus-visible:outline-ring"><TrashIcon aria-hidden /></button>
                  </div>
                  <p className="mt-2 text-xs text-muted-foreground">{line.color} · {line.size}</p>
                  {!line.canCheckout ? <p className="mt-2 flex items-center gap-1 text-xs text-destructive"><WarningCircleIcon aria-hidden /> Stock unavailable</p> : null}
                  <div className="mt-4 flex items-center justify-between gap-3">
                    <div className="flex h-8 items-center border border-border">
                      <button type="button" aria-label={`Decrease quantity for ${line.product.name}`} onClick={() => updateQuantity(line.id, line.quantity - 1)} className="grid size-8 place-items-center"><MinusIcon aria-hidden /></button>
                      ...
                      <button type="button" aria-label={`Increase quantity for ${line.product.name}`} disabled={line.quantity >= line.availableStock} onClick={() => updateQuantity(line.id, line.quantity + 1)} className="grid size-8 place-items-center disabled:opacity-35"><PlusIcon aria-hidden /></button>
                    </div>
        ...
        {cartLines.length ? (
          <SheetFooter className="border-t border-border bg-secondary/25 p-5">
            ...
            <p className="mb-3 text-xs leading-5 text-muted-foreground">Shipping and vouchers are calculated in the full shopping bag.</p>
