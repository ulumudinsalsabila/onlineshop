...
export function ProductGallery({ product }: { product: CatalogProduct }) {
  ...
  return (
    <div className="min-w-0">
      <div className="-mx-(--container-gutter) flex snap-x snap-mandatory overflow-x-auto [scrollbar-width:none] md:hidden [&::-webkit-scrollbar]:hidden" aria-label={`Galeri ${product.name}`}>
        {images.map((image, index) => <div key={image} className="relative aspect-[4/5] w-[92vw] shrink-0 snap-center bg-secondary"><Image src={image} alt={`${product.name}, tampilan ${index + 1}`} width={1254} height={1254} priority={index === 0} sizes="92vw" className="absolute inset-0 size-full object-cover" /></div>)}
      </div>

      <div className="hidden grid-cols-[5.5rem_minmax(0,1fr)] gap-4 md:grid">
        <div className="space-y-3" aria-label="Pilih gambar">
          {images.map((image, index) => <button key={image} type="button" onClick={() => setActive(index)} aria-label={`Tampilkan gambar ${index + 1}`} aria-pressed={active === index} className="relative aspect-[4/5] w-full overflow-hidden bg-secondary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring"><Image src={image} alt="" width={1254} height={1254} sizes="88px" className="absolute inset-0 size-full object-cover" /><span className={`absolute inset-0 border-2 transition-colors ${active === index ? "border-primary" : "border-transparent"}`} /></button>)}
        </div>
        <div
        ...
        >
          <AnimatePresence mode="wait" initial={false}>
            <m.div key={images[active]} initial={{ opacity: 0.25 }} animate={{ opacity: 1 }} exit={{ opacity: 0.2 }} className="absolute inset-0">
              <Image src={images[active]} alt={`${product.name}, tampilan ${active + 1}`} width={1254} height={1254} priority={active === 0} sizes="(max-width: 1200px) 55vw, 45vw" className="absolute inset-0 size-full object-cover transition-transform duration-500 group-hover:scale-[1.65]" style={{ transformOrigin: origin }} />
            </m.div>
          </AnimatePresence>
