...
export function WishlistPage() {
  ...
  return (
    <Container className="py-10 sm:py-14">
      ...
      <div className="mt-8"><p className="text-[0.625rem] font-semibold tracking-[0.18em] text-muted-foreground uppercase">Saved for later</p><h1 className="mt-2 font-serif text-(length:--text-heading-1)">Your wishlist</h1><p className="mt-4 text-sm text-muted-foreground">{products.length} piece tersimpan</p></div>
      {!products.length ? <EmptyState icon={HeartIcon} className="mt-10 min-h-[28rem]" title="Belum ada wishlist" description="Gunakan ikon hati pada produk untuk menyimpan piece yang ingin Anda lihat kembali." action={<Button asChild><Link href="/products">Explore products</Link></Button>} /> : (
          ...
          <AnimatePresence initial={false}>
            {products.map((product) => (
              <m.article key={product.id} layout initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.94 }}>
                ...
                <Button className="mt-4 w-full" disabled={product.stock < 1 || product.soldOut} onClick={() => { const moved = moveWishlistToCart(product.id); toast(moved ? "Dipindahkan ke shopping bag" : "Produk sedang tidak tersedia"); }}>{product.soldOut ? "Sold out" : <><ShoppingBagIcon aria-hidden /> Move to cart</>}</Button>
              </m.article>
            ))}
          </AnimatePresence>
        </div>
