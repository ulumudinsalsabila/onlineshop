...
export function RecordEditor({ title, endpoint, fields, initial }: { title: string; endpoint: string; fields: RecordField[]; initial: Record<string, string | number | boolean | null> }) {
  ...
  async function save() {
    ...
    try {
      ...
      if (!response.ok || !result.success) throw new Error(result.error?.message ?? "Could not save the data.");
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button size="icon-sm" variant="outline" aria-label={`Edit ${title}`}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Changes are saved directly to the database through the backend API.</DialogDescription>
        </DialogHeader>
        <div className="grid gap-4 sm:grid-cols-2">
          {fields.map((field) => (
            <div key={field.name} className={field.type === "checkbox" ? "flex items-center gap-3 pt-7" : ""}>
              {field.type === "checkbox" ? (
                <>
                  <Checkbox
                    id={`${title}-${field.name}`}
        ...
        <Button disabled={busy} onClick={() => void save()}>
          {busy && <SpinnerGapIcon className="animate-spin" aria-hidden />}Save
        </Button>
      </DialogContent>
    </Dialog>
  );
