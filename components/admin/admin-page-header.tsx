...
export function AdminPageHeader({ title, description, action, breadcrumbs = [] }: { title: string; description?: string; action?: React.ReactNode; breadcrumbs?: { label: string; href?: string }[] }) {
  return <header className="mb-6"><nav aria-label="Breadcrumb"><ol className="mb-3 flex flex-wrap items-center gap-2 text-xs text-[#827b70]"><li><Link href="/admin" className="hover:text-foreground">Admin</Link></li>{breadcrumbs.map((item) => <li key={item.label} className="flex items-center gap-2"><CaretRightIcon aria-hidden />{item.href ? <Link href={item.href}>{item.label}</Link> : <span aria-current="page">{item.label}</span>}</li>)}</ol></nav><div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-2xl font-semibold tracking-tight text-[#292722] sm:text-3xl">{title}</h1>{description && <p className="mt-1.5 max-w-2xl text-sm leading-6 text-[#746e64]">{description}</p>}</div>{action}</div></header>;
}
